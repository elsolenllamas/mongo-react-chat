...
import { CHAT_ERROR, FETCH_CONVERSATIONS, FETCH_CHANNELS, FETCH_RECIPIENTS, START_CHANNEL, START_CONVERSATION, SEND_REPLY, FETCH_SINGLE_CONVERSATION } from './types';
...
export function startChannel({ channelName }) {
  const data = { composedMessage };
  const url = `/chat/new/channel/${channelName}`;
  return (dispatch) => { 
    postData(START_CHANNEL, CHAT_ERROR, true, url, dispatch, data);
...
export function fetchChannels() {
  const url = '/chat/channels/list';
  return dispatch => getData(FETCH_CHANNELS, CHAT_ERROR, true, url, dispatch);
}
